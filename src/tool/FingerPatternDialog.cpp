...
static QByteArray Packfinger() {
    ...
    for(int n = 0; n < MAX_FINGER_PATTERN; n++) {
        ...
        for(int m = 0; m < (MAX_FINGER_NOTES + 4); m++) {
            ...
            a.append((char) finger_patternUP[n][m]);
            a.append((char) finger_patternDOWN[n][m]);
        }
...
// UP ZONE NOTE OFF
        ...
        if(map_key_offUP[n]) {
            ...
            if(!finger_switch_playUP) {
                ...
                message.clear();
                message.emplace_back((char) 0x80 + ch_up);
                message.emplace_back((char) n);
                message.emplace_back((char) 0);
            } else {

                messageByte.clear();
                messageByte.append((char) 0x80 + ch_up);
                messageByte.append((char) n);
                messageByte.append((char) 0);
            }
...
// DOWN ZONE NOTE OFF
        ...
        if(map_key_offDOWN[n]) {
            ...
            if(!finger_switch_playDOWN) {
                ...
                message.clear();
                message.emplace_back((char) 0x80 + ch_down);
                message.emplace_back((char) n);
                message.emplace_back((char) 0);
            } else {

                messageByte.clear();
                messageByte.append((char) 0x80 + ch_down);
                messageByte.append((char) n);
                messageByte.append((char) 0);
            }
...
// UP ZONE NOTE ON
        ...
        if(map_key_onUP[n]) {
            ...
            if(!finger_switch_playUP) {
                ...
                message.clear();
                message.emplace_back((char) 0x90 + ch_up);
                message.emplace_back((char) n);
                message.emplace_back((char) map_key_velocityUP[map_keyUP[n] & 127]);
            } else {

                messageByte.clear();
                messageByte.append((char) 0x90 + ch_up);
                messageByte.append((char) n);
                messageByte.append((char) map_key_velocityUP[map_keyUP[n] & 127]);
            }
...
// DOWN ZONE NOTE ON
        ...
        if(map_key_onDOWN[n]) {
            ...
            if(!finger_switch_playDOWN) {
                ...
                message.clear();
                message.emplace_back((char) 0x90 + ch_down);
                message.emplace_back((char) n);
                message.emplace_back((char) map_key_velocityDOWN[map_keyDOWN[n] & 127]);
            } else {

                messageByte.clear();
                messageByte.append((char) 0x90 + ch_down);
                messageByte.append((char) n);
                messageByte.append((char) map_key_velocityDOWN[map_keyDOWN[n] & 127]);
            }
